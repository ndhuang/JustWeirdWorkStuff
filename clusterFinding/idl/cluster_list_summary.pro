...
printf, tex, ""
printf, text, "name                sig        ra         dec        radius"
...
for i = 0, N - 1 do begin
   ...
   name = radec_to_spt_name(ra[i], dec[i])
   tex_name = radec_to_spt_name(ra[i], dec[i], /latex)

   if outtemp_2band[i].peaksig gt 4.0 then begin
      if i mod 40 eq 0 then begin
         ...
         printf, tex, "\begin{tabular}{|c|c|c|c|c|l|}"
         printf, tex, "\hline"
         printf, tex, "Name & Significance (sigma) & RA (degrees) & dec (degrees) & radius (arcmin) & notes \\"
         printf, tex, "\hline"
      endif
      printf, tex, tex_name + " & " + sig_s + " & " + ra_s + " & " + dec_s + " & " + rad_s + " & \\"
   endif
   printf, text, name, + "  " + sig_s + "    " + ra_s + "    " + dec_s + "     " + rad_s
endfor
printf, tex, "\hline"
printf, tex, "\end{tabular}"
close, tex
